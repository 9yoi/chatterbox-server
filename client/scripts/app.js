  ...
  fetch: function(animate) {
    $.ajax({
      ...
      success: function(data) {
        ...
        if (!newData.results || !newData.results.length) { return; }

        // Store messages for caching later
        app.messages = newData.results;

        // Get the last message
        var mostRecentMessage = newData.results[newData.results.length - 1];
 
        // Only bother updating the DOM if we have a new message
        if (mostRecentMessage.objectId !== app.lastMessageId) {

          // Update the UI with the fetched rooms
          app.renderRoomList(newData.results);

          // Update the UI with the fetched messages
          app.renderMessages(newData.results, animate);
